...
Story points are used by the Scrum team to measure the team's velocity (how many SP the team completes in one sprint on average) and forecast accordingly. Story points cannot be used to gauge individuals and must not be equated to man-days.
...
**Why it does not make sense:**
Despite the often-held belief that they are or can be, requirements are never fixed. It is impossible to determine how long a functionality will take to build, if the definition of said functionality (scope) changes. If the scope is fluid, then the duration and the cost to complete the implementation of the scope cannot be fixed, or even predicted with much accuracy.
