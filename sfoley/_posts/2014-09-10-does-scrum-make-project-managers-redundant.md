...
The combination can work, but it is important to note the differences in the roles. The Scrum Master is solely concerned with the efficient execution of the Scrum process for a team and must take a coaching approach to problems rather than a command approach. There also may be a natural tension between the roles e.g. the cadence of Scrum reporting and programme level reporting may not align well, so anyone combining the roles needs to be able to wear both hats and resolve these tensions.  

### Product Owner and Project Manager

The Product Owner role has elements of overlap with project management. However, typically a Product Owner requires decision making authority for requirements and a level of domain expertise that Project Managers rarely possess. Combining the roles can lead to the Project Manager effectively being a proxy Product Owner which can slow down the whole process of requirements elicitation and prioritisation. In addition, many Product Owners are taken from business lines and may not be able to bring the range of project management skills required