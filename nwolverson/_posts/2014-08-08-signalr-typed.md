...
The client is written using FunScript (the source is in F# and is translated to JavaScript via the FunScript compiler). The client uses the FunScript bindings for SignalR, and has a reference to the server DLL. The client references the SignalRProvider, which searches for hubs (by attribute) in the referenced DLLs, and exposes these as new types:
